...
type
  // Структура для зберігання загальної інформації про модель
  TModelInfo = record
    ModelName: string;
    FullName: string;
    Category: string;
  ...
  // Оновлена структура для пристрою, що містить індекс моделі
  TDeviceInfo = record
    ID: longword;
    ModelCode: string;
    ModelIndex: byte; // Byte (0..255) достатньо для індексів
  end;
...
{
  Функція для пошуку повної інформації про пристрій за його ID.
  Параметри:
    - aID: ID пристрою, який потрібно знайти.
    - aFullInfo: змінна, в яку буде записано повну інформацію (якщо знайдено).
  Повертає:
    - True, якщо пристрій з таким ID існує.
    - False, якщо пристрій не знайдено.
}
...
var
  ...
  Model: TModelInfo;
begin
  Result := False; // За замовчуванням пристрій не знайдено
  aFullInfo := Default(TFullDeviceInfo); // Очищуємо вихідну структуру

  // 1. Шукаємо пристрій в основному масиві
  for Device in BB10Devices do
  begin
    if Device.ID = aID then
    begin
      // 2. Якщо знайшли, отримуємо інформацію про модель за індексом
      Model := DeviceModels[Device.ModelIndex];

      // 3. Заповнюємо вихідну структуру TFullDeviceInfo
      ...
      Result := True; // Повідомляємо про успіх
      Exit; // Виходимо з функції, оскільки пристрій вже знайдено
    end;
  end;
end;
